...
function(CmDepBuildExternal name version source_dir cmake_args build_targets)
    CmDepInstallDir(INSTALL_INT_CONFIG ${name} ${version})
    ...
    if (CMAKE_C_COMPILER)
        set(cmake_arg_complete "${cmake_arg_complete}-DCMAKE_C_COMPILER=${CMAKE_C_COMPILER};")
    endif ()
    set(cmake_arg_complete "${cmake_arg_complete}-DCMAKE_C_FLAGS=${CMAKE_C_FLAGS};-DCMAKE_CXX_FLAGS=${CMAKE_CXX_FLAGS}")

    if (CMAKE_POSITION_INDEPENDENT_CODE)
        set(cmake_arg_complete "${cmake_arg_complete};-DCMAKE_POSITION_INDEPENDENT_CODE=ON")
    endif ()
    ...
    foreach (build_target ${build_targets})
        if (TARGET ${build_target})
            ...
            if (WIN32 AND (target_type STREQUAL SHARED_LIBRARY))
                get_target_property(imported_implib_debug ${build_target} CMDEP_IMPLIB_DEBUG)
                list(APPEND build_byproducts ${imported_implib_debug})
                get_target_property(imported_implib_release ${build_target} CMDEP_IMPLIB_RELEASE)
                if (NOT "${imported_implib_release}" STREQUAL "${imported_implib_debug}")
                    list(APPEND build_byproducts ${imported_implib_release})
                endif ()
            else ()
                get_target_property(imported_location_debug ${build_target} CMDEP_LOCATION_DEBUG)
                list(APPEND build_byproducts ${imported_location_debug})
                get_target_property(imported_location_release ${build_target} CMDEP_LOCATION_RELEASE)
